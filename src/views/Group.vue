  ...
  <div class="w-full flex flex-col items-center bg-gray-200">
    ...
    <div class="flex w-full justify-center mt-6">
      <div class="w-2/5 mr-10 bg-white p-6 flex flex-col items-center">
        ...
        <p class="w-full text-left mt-2 text-xs text-gray-500">由{{agency.name}}组织</p>
      ...
      <div class="w-1/5 bg-white flex flex-col p-6 items-start h-48">
        <span class="text-sm font-semibold mb-2">
          价格：<span class="text-red-600 font-bold">{{ group.price }}￥</span>
        </span>
        <span class="text-sm font-semibold mb-1">旅游公司：{{agency.name}}</span>
        <span class="text-sm font-semibold mb-1">公司电话：<span class=" text-teal-500 font-bold">{{agency.telephone}}</span></span>
        <div class="text-left text-sm font-semibold mb-4 w-full overflow-hidden" style="text-overflow: ellipsis;white-space: nowrap;">公司简介：{{agency.brief}}</div>
...
export default {
  ...
  created(){
    ...
    let that = this;
    let url = this.$state.getUrl()+'agency/get';
    let info = {account: this.group.agency_account}
    axios.post(url,info)
    .then(function(res) {
      if(res.data.result){
        that.agency = res.data.agency_info;
      }
    })
    .catch(function(error) {
      console.log(error);
